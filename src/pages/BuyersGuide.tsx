  ...
  useEffect(() => {
    if (apiSearchResults && apiSearchResults.length > 0) {
      ...
      const transformedResults: CarData[] = apiSearchResults.map((car, index) => ({
        ...
        horsepower: car.cylinders ? `${car.cylinders * 50}-${car.cylinders * 75} hp` : '200-300 hp',
        horsepowerTorque: car.cylinders ? `${car.cylinders} cylinders` : undefined,
        fuelType: car.fuel_type || 'Gasoline',
        drivetrain: car.drive || 'FWD',
        motorTrendScore: (Math.random() * 2 + 7).toFixed(1),
        motorTrendRank: `#${index + 1}`,
        motorTrendCategoryRank: true,
        isNew: false,
        msrp: 'Contact Dealer',
        mpg: car.fuel_type === 'Electric' ? undefined : `${20 + Math.floor(Math.random() * 15)} city / ${30 + Math.floor(Math.random() * 10)} hwy`,
