...
const Index = () => {
  ...
  const [content, setContent] = useState({
    ...
    // Prepopulate with all available articles
    newCars: mockNewCars,
    // Prepopulate with all available new cars
    usedCars: mockUsedCars,
    // Prepopulate with all available used cars
    photos: mockPhotos,
    // Prepopulate with all available photos
          ...
          <div className="flex-1 overflow-y-auto" ref={chatContainerRef}>
            <div className="max-w-[980px] mx-auto w-full pb-32 px-0 py-[16px]">
              {searchHistory.length === 0 ? <div className="flex flex-1 flex-col items-center justify-center space-y-6 px-[32px] py-[32px]">
                  ...
                  {searchHistory.map((result, index) => <div key={result.id} className="space-y-4" ref={index === searchHistory.length - 1 ? latestResultRef : undefined}>
