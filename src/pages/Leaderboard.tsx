...
const prizeIcons: Record<string, JSX.Element> = {
  "Special Badge": <Trophy className="w-5 h-5 text-orange-500" />,
  // Bragging rights
      ...
      <Card className="bg-motortrend-dark text-white">
        ...
        <CardContent>
          <table className="w-full text-left mt-4">
            ...
            <tbody>
              {users.map(user => <tr key={user.id} className="border-b border-gray-800 hover:bg-gray-900 transition">
                  ...
                  <td className="flex items-center gap-3 py-2">
                    ...
                    <span>{user.name}</span>
                    {user.weeklyWinner && <span className="ml-2 px-2 py-1 rounded bg-yellow-500 text-black text-xs font-semibold">Winner</span>}
                  </td>
                  <td>{user.points}</td>
                  <td className="flex gap-1">
                    {user.badges.map(badge => <span key={badge} title={badge} className="inline-block">{badgeIcons[badge]}</span>)}
                  </td>
                  <td>
                    {user.prize && <span className="flex items-center gap-1">
