...
const ArticleDetail: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <article className="prose prose-lg max-w-none text-left">
          {mockContent.sections.map((section, index) => {
            switch (section.type) {
              case 'paragraph':
                return (
                  <p key={index} className="mb-6 text-gray-700 leading-relaxed text-lg text-left">
                    {section.content}
                  </p>
                );
              case 'heading':
                return (
                  <h2 key={index} className="text-3xl font-bold mt-12 mb-6 text-gray-900 relative text-left">
        ...
        <section className="mt-16 p-8 bg-white rounded-xl shadow-sm border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {mockArticles.slice(0, 3).filter(a => a.id !== id).map(relatedArticle => (
              <Link key={relatedArticle.id} to={`/article/${relatedArticle.id}`} className="block group hover:shadow-lg transition-all duration-300 rounded-lg overflow-hidden border hover:border-motortrend-red/20">
      ...
      {showScrollTop && (
        <button onClick={scrollToTop} className="fixed bottom-8 right-8 bg-motortrend-red text-white p-3 rounded-full shadow-lg hover:bg-motortrend-dark transition-all duration-300 z-40" aria-label="Scroll to top">
          <ChevronUp size={20} />
        </button>
      )}
    </div>
