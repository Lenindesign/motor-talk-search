...
import { usePersonalization, ThemeType, FontSizeType } from "../contexts/PersonalizationContext";
...
const PersonalizationDialog: React.FC<PersonalizationDialogProps> = ({
  ...
  const categories = ["Sports Cars", "Electric Vehicles", "Muscle Cars", "Reviews", "Luxury", "Trucks", "Hypercars", "Comparisons", "SUVs", "Classics", "Off-Road", "Industry News"];
  const manufacturers = ["Porsche", "Audi", "Toyota", "Lamborghini", "Acura", "BMW", "Tesla", "Honda", "Ferrari", "Nissan", "Mercedes-Benz", "Ford", "Chevrolet", "Lexus", "Hyundai"];
  ...
  const handleCategoryToggle = (category: string) => {
    setTempPreferences(prev => {
      ...
      return {
        ...prev,
        favoriteCategories: currentCategories
      };
    });
  };
  const handleManufacturerToggle = (manufacturer: string) => {
    setTempPreferences(prev => {
      ...
      return {
        ...prev,
        favoriteManufacturers: currentManufacturers
      };
    });
  };
  const handleSave = () => {
