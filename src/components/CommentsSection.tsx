...
import { ThumbsUp, ThumbsDown, Flag, MoreHorizontal, Bookmark } from 'lucide-react';
...
export const CommentsSection: React.FC<CommentsSectionProps> = ({
  ...
  const CommentCard: React.FC<{ comment: Comment; isReply?: boolean; articleId: string }> = ({ comment, isReply, articleId }) => {
    const { isSaved, toggleSave } = useCardSave({
      ...
      metadata: {
        ...
        replies: comment.replies?.length.toString() || '0'
      }
    });
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button 
              ...
              className={`flex items-center gap-1 typography-small sm:typography-caption transition-colors ${
                isSaved ? 'text-motortrend-red' : 'text-neutral-3 hover:text-neutral-1'
              }`}
              aria-label={isSaved ? "Unsave comment" : "Save comment"}
            >
              <Bookmark size={14} className={`sm:w-4 sm:h-4 ${isSaved ? 'fill-current' : ''}`} />
          ...
          {comment.replies?.map((reply) => (
            <CommentCard key={reply.id} comment={reply} isReply articleId={articleId} />
          ))}
        </div>
      </div>
    );
