  ...
  return <section className="space-content">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <h2 className="typography-display text-neutral-1 text-2xl">Latest Reviews</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {reviewItems.map(item => <article key={item.id} className="group">
            <Link to={`/article/${item.id}`} className="block">
              <div className="bg-white rounded-2xl overflow-hidden shadow-modern hover:shadow-modern-lg transition-all duration-300 group-hover:-translate-y-1">
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  {item.rating && <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm rounded-lg px-3 py-2">
